...
def test_signup_rejects_duplicate_student_in_same_activity():
    # Arrange
...
def test_signup_rejects_student_registered_for_another_activity():
    # Arrange
    current_activity = "Chess Club"
    other_activity = "Basketball Team"
    email = activities[current_activity]["participants"][0]
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == (
        "Student is already signed up for another activity"
    )
